...
function getDeviceType () {
  ...
  var ESPNOmniture = {
    omniture: {
      ...
      prop16: window.location.href.split(/[?#]/)[0],
...
function analyticsInit () {
  ...
  $element = $( element );
    ...
    window.updateOmnitureData = function() {
      ...
      ESPNOmniture.omniture.prop16 = window.location.href.split(/[?#]/)[0];
  ...
  if (related) {
    window.addEventListener('scroll', function(e) {
      if (!window.taboolaAdded && inView(related, window, 50)) {
...
function inView(el, window, offsetTop) {
  var rect = el.getBoundingClientRect();

  // This is an optional offset if this el should be considered "in view" before you've actually scrolled that far
  offsetTop = offsetTop || 0;

  return rect.bottom > 0 &&
         rect.right > 0 &&
         rect.left < (window.innerWidth || window.document.documentElement.clientWidth) &&
         rect.top < (window.innerHeight || window.document.documentElement.clientHeight) + offsetTop;
}

function initAds() {
  if (window.___skipShowAds || !document.getElementById('ad') || window.adsInititalized) return;
  window.adsInititalized = true;

  var div = document.createElement('div');
  div.setAttribute('id', 'div-gpt-ad-1558036340566-0');
  document.getElementById('ad').appendChild(div);

  var targeting = adConfig && adConfig.targeting ? adConfig.targeting : '/21783347309/abc-news/fivethirtyeight.com/web/projects/';
  var sizes = getDeviceType() == 'Smartphone' ? [[300, 50], [320, 50]] : [[728, 90], [970, 250], [970, 66]];

  getScript('https://www.googletagservices.com/tag/js/gpt.js', function() {
    googletag.cmd.push(function() {
      googletag.defineSlot(targeting, sizes, 'div-gpt-ad-1558036340566-0').addService(googletag.pubads());
      googletag.pubads().enableSingleRequest();
      googletag.pubads().collapseEmptyDivs();
      googletag.enableServices();
    });
    googletag.cmd.push(function() { googletag.display('div-gpt-ad-1558036340566-0'); });
  });

}

function refreshAds() {
  if (window.___skipShowAds || !document.getElementById('ad')) return;
