...
function ready(error, json, inputData, dataInput) {
	...
	var dataInput = dataInput.slice(dataInput.length - 171, dataInput.length)
	...
	states.forEach((d, i) => {
		...
		var gopVote = stateData.filter(d => d.party == "gop")[0].proj_vote
		var demVote = stateData.filter(d => d.party == "dem")[0].proj_vote
		var thirdVote = stateData.filter(d => d.party == "third")[0].proj_vote
		...
		data.push({
			state: state,
			electoralVotes: +stateData[0].electoral_vote,
			gopVote: +gopVote,
			demVote: +demVote,
			thirdVote: +thirdVote,
			variance: +variance,
	...
	var avgs = []
	var stavgs = []
	update(data, avgs, mapType);
	function update(input, avgs, type) {
		var averages = avgs
		...
		var data = input
		var swings = {
			national: jStat.normal.inv((Math.random()), 0, 3),
		...
		data.forEach((d, i) => {
			...
			d.gopSim = jStat.normal.inv(Math.random(), d.gopVote, 2 * d.variance) + (swings.national * d.variance) / 2 +0
			d.demSim = jStat.normal.inv(Math.random(), d.demVote, 2 * d.variance) - (swings.national * d.variance) / 2
