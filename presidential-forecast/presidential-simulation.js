...
function ready(error, json, inputData, dataInput) {
	...
	function update(input, avgs, type, num) {
		...
		for (let a = 0; a < num; a++) {
			...
			var data = input
			var swings = {
				national: jStat.normal.inv((Math.random()), 0, 5.5),
			...
			data.forEach((d, i) => {
				...
				d.gopSim = jStat.normal.inv(Math.random(), d.gopVote, 1.5 * d.variance) + (swings.national * d.variance) / 2 + swings[d.region] / 2
				d.demSim = jStat.normal.inv(Math.random(), d.demVote, 1.5 * d.variance) - (swings.national * d.variance) / 2 - swings[d.region] / 2
