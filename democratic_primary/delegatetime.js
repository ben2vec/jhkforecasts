...
d3.csv("delegatetime.csv", function (error, data) {
  ...
  var xScaletwo = d3.scaleTime()
    .domain([mindate, maxdate])
    .range([0, width])



  var yScaletwo = d3.scaleLinear()
  ...
  var xAxis = d3.axisBottom(xScaletwo)
  ...
  svg.append("g")
    ...
    .call(g => {
      var years = xScaletwo.ticks(d3.timeYear.every(1))
      var xshift = 0
      g.selectAll("text")
  ...
  svg.append("line")
    .attr("x1", "0")
    .attr("x2", "850")
    .attr("y1", yScaletwo(1990))
    .attr("y2", yScaletwo(1990))
    .attr("stroke", "grey")
  ...
  var line = d3.line()
    ...
    .x(d => xScaletwo(d.date))
    .y(d => yScaletwo(d.percentage))
  ...
  function updateChart(dataPoint) {
    ...
    yScaletwo.domain([0, maxYValu])
    yAxis.transition().duration(1000).call(d3.axisLeft(yScaletwo).ticks(5)).call(g => {
      g.selectAll("text")
    ...
    glinestwo.select('.line') //select line path within line-group (which represents a vehicle category), then bind new data 
      .data(res_nested)
    ...
    mouseGtwo.selectAll('.mouse-per-line')
      .data(res_nested)

    mouseGtwo.on('mousemove', function () {
  ...
  function renderChart(dataPoint) {
    ...
    glinestwo = lines.selectAll('.line-group')
    ...
    //.attr("class", "circle")  
    //.append("circle")
    //.attr("cx", d => xScaletwo(d.date))
    //.attr("cy", d => yScaletwo(d.percentage))
    //.attr("r", 2)

    // CREATE HOVER TOOLTIP WITH VERTICAL LINE //
    tooltip1 = d3.select("#chart1").append("div")
      .attr('id', 'tooltip1')
    ...
    mouseGtwo = svg.append("g")
      .attr("class", "mouse-over-effects");

    mouseGtwo.append("path") // create vertical line to follow mouse
    ...
    var mousePerLinetwo = mouseGtwo.selectAll('.mouse-per-line')
      .data(res_nested)
      .enter()
      .append("g")
      .attr("class", "mouse-per-line");

    mousePerLinetwo.append("circle")
      .attr("r", 4)
      .style("stroke", "white"
      )
    ...
    mouseGtwo.append('svg:rect') // append a rect to catch mouse movements on canvas
      .attr('width', width)
      .attr('height', height)
      .attr('fill', 'none')
      ...
      .on('mousemove', function () { // update tooltip content, line, circles and text when mouse moves
        ...
        d3.selectAll(".mouse-per-line")
          .attr("transform", function (d, i) {
            var xDate = xScaletwo.invert(mouse[0]) // use 'invert' to get date corresponding to distance from mouse position relative to svg
            var bisect = d3.bisector(function (d) { return d.date; }).left // retrieve row index of date on parsed csv
            var idx = bisect(d.values, xDate);

            d3.select(".mouse-line")
              .attr("d", function () {
                var data = "M" + xScaletwo(d.values[idx].date) + "," + (height);
                data += " " + xScaletwo(d.values[idx].date) + "," + 0;
                return data;
              });
            return "translate(" + xScaletwo(d.values[idx].date) + "," + yScaletwo(d.values[idx].percentage) + ")";
  ...
  function updateTooltipContent(mouse, res_nested) {
    ...
    sortingObj = []
    res_nested.map(d => {
      var xDate = xScaletwo.invert(mouse[0])
    ...
    tooltip1.html(sortingObj[0].month + "-" + sortingObj[0].day)
      .style('display', 'inline')
